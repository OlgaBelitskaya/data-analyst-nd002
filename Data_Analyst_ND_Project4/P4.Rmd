...
pl1 <- plot_ly(centrobank, x=EUR_978, y=USD_840, 
               z=gold, color = gold, 
               text = paste(date), 
               colors = 'Spectral', type="scatter3d", mode="markers", 
               marker = list(size = 8, symbol = "circle-open"))
pl2 <- layout(pl1, title = "Prices for Gold and Rates for USD, EUR",
         scene = list(
           xaxis = list(title = "EUR"), 
           yaxis = list(title = "USD"), 
           zaxis = list(title = "gold")))
...
pl3 <- plot_ly(centrobank, x=gold, y=silver, 
               z=dual_currency_basket, 
               color = dual_currency_basket, 
               text = paste(date), colors = 'Spectral', 
               type="scatter3d", mode="markers", 
               marker = list(size = 8, symbol = "diamond-open"))
pl4 <- layout(pl3, title = "Prices for Gold, Silver and Dual Currency Basket",
         scene = list(
           xaxis = list(title = "gold"), 
           yaxis = list(title = "silver"), 
           zaxis = list(title = "dual currency basket")))
pl4
```
...
```{r echo=FALSE, message=FALSE, warning=FALSE,echo=FALSE, Plot_One_1}
centrobank_y <- aggregate(centrobank[, 2:18], list(centrobank$year), mean)
centrobank_y$silver100 <- 100*centrobank_y$silver
```

```{r fig.width=10, fig.height=7, echo=FALSE, message=FALSE, warning=FALSE,echo=FALSE, Plot_One_2}
...
##### The artificial variables combining in one plot show us a very interesting tendency. Yes, precious metals are traditionally considered one of the best investment funds. Their cost in the long term has been steadily increased in monetary terms. But the ratio of the prices for precious metals to the value of the dual currency basket falls from 2012 to 2016. This is particularly noticeable when the four main metals are analyzed together.

### 6.2.1 Plot Two

##### Let's look at the same time to the changes in exchange rates and the prices of precious metals.
...
p22 <- ggplot(data = centrobank, aes(x = date)) + 
  ...
  theme(axis.title.y = 
          element_text(colour="grey20",size=16,
...
p23 <- ggplot(data = centrobank, aes(x = date)) + 
  ...
  theme(axis.title.y = 
          element_text(colour="grey20",size=16,angle=90,
...
##### Let us estimate the difference in the behavior of prices for precious metals in short and long terms.
...
##### I want to note also that the color decisions really helped me in the research. They allowed to group not only the variables on individual charts, but graphics in general on certain groups.
