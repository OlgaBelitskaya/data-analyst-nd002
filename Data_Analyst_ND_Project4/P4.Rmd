...
```{r echo=FALSE, message=FALSE, warning=FALSE, comment='', Correlation_Plots_3}
...
ggsave('/Users/olgabelitskaya/version-control/reflections-ud651/figure-html/Bivariate_Plots_14-1.png', g5, width = 18, height = 24) 
```
...
p21<- ggplot(data = centrobank, aes(x = date)) + 
...
g11 <- grid.arrange(p21, p22, ncol=1)
...
m1 <- lm(I(dual_currency_basket) ~ I(USD_840), data = centrobank)
m2 <- update(m1, ~ . + EUR_978)
m4 <- update(m3, ~ . + gold)
m5 <- update(m4, ~ . + platinum)
mtable(m1, m2, m3, m4)
```
